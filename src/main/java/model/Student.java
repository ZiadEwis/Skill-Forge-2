...
public class Student extends User {
    ...
    // New method: Get number of attempts for a specific lesson quiz
    public int getQuizAttemptCount(String courseId, String lessonId) {
        return (int) quizAttempts.stream()
                .filter(attempt -> attempt.getCourseId().equals(courseId)
                        && attempt.getLessonId().equals(lessonId))
                .count();
    }

    // New method: Check if student has passed a quiz
    public boolean hasPassedQuiz(String courseId, String lessonId) {
        return quizAttempts.stream()
                .anyMatch(attempt -> attempt.getCourseId().equals(courseId)
                        && attempt.getLessonId().equals(lessonId)
                        && attempt.isPassed());
    }
}